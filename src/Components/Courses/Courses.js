...
function Courses() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/courses/course?limit=${showCount}`)
      .then((res) => {
      ...
      <div class="max-w-screen-2xl mx-auto p-5 sm:p-10 md:p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
          {products.slice(0, showCount).map((product, index) => (
            <div class="rounded overflow-hidden shadow-xl shadow-blue-200 flex flex-col">
              <div class="relative">
                <a href="!#">
                  <img
                    class="w-full"
              ...
              <div class="px-6 py-4 mb-auto">
                ...
                <p class="text-gray-700 text-xl font-bold  text-justify">
                  {product.description}
                </p>
              </div>
        ...
        <div className="flex items-center  justify-center my-8 w-full">
          <Link to="/course">
            <button
              ...
              className="hover:bg-gray-700 bg-indigo-500  shadow-md shadow-gray-400  tracking-widest font-assistant  lg:text-2xl md:text-lg text-sm rounded f-m-m font-semibold text-white focus:outline-none lg:px-12 px-6 lg:py-6 py-3 xl:leading-4"
            >
              Browse More
            </button>
          </Link>
